...
class Model(object):
    ...
    def _build_model(self):
        ...
        pooled = tf.nn.max_pool(contex,
                                ksize=[1, self.max_seq_len, 1, 1],
                                strides=[1, 1, 1, 1],
                                padding='VALID',
                                name="pool")
        # [batch x rcnn_size]
        # rcnn_size = self.rnn_size * 2 + self.word_dim
        rcnn_pooled = tf.squeeze(pooled)

        fc1 = tf.layers.dense(rcnn_pooled, self.rnn_size, activation=tf.nn.relu)
