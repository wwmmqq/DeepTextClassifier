...
class Model(object):
    def __init__(self, config):
        ...
        self.att_hops = 5  # this for multiple hops of attention
    ...
    def _build_model(self):
        ...
        b_outputs, b_states = tf.nn.bidirectional_dynamic_rnn(rnn_cell_fw,
                                                              ...
                                                              dtype=tf.float32)
        # bi_rnn_out : (batch_size, max_time, cell_bw.output_size x 2)
        ...
        for i in range(self.att_hops):
            ...
            self.att_probs.append(att_prob)
        # multi_sent : self.att_hops x [batch x context_size]
        multi_sentence = tf.concat(multi_sent, 1)  # [batch x (self.att_hops * context_size)]
    ...
    def _set_cost_and_optimize(self):
        ...
        # penalization of multi hops attention
        # atts shape: (batch, self.att_hops, max_time)
        atts = tf.stack(self.att_probs)
        atts_shape = tf.shape(atts)  # (batch, self.att_hops, max_time)
        # t_atts = tf.reshape(atts, [atts_shape[0] * atts_shape[1], atts_shape[2]])
        # atts * tf.transpose(atts, perm=[1, 0]) - tf.eye()
        t_atts = tf.transpose(atts, perm=[0, 2, 1])
        p = tf.matmul(atts, t_atts)  # shape: (batch, self.att_hops, self.att_hops)
        # p = tf.subtract(p, tf.eye(atts_shape[1]))
        p = p - tf.eye(atts_shape[1])
        self.cost = softmax_cost + 0.1*tf.nn.l2_loss(p)
