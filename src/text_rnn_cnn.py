...
class Model(object):
    ...
    def _build_model(self):
        ...
        outputs, states = tf.nn.dynamic_rnn(cell_fw, embedded_seq, self.in_len, dtype=tf.float32)
        rnn_seq = tf.expand_dims(states, -1)  # shape: (batch x seq x rnn_size x 1)
        ...
        for i in range(len(kernel_sizes)):
            with tf.name_scope("convolution-max-pool-%s" % kernel_sizes[i]):
                ...
                conv = tf.nn.conv2d(rnn_seq, w_filter, strides=[1, 1, 1, 1], padding="VALID", name="conv")
        ...
        fc1 = tf.layers.dense(cnn_pool_drop, self.mlp_size, activation=tf.nn.relu)
        self.logits = tf.layers.dense(fc1, self.class_num)
