...
class Model(object):
    ...
    def _build_model(self):
        ...
        bi_rnn_out = tf.concat(b_states, axis=-1)

        # self attention
        # [batch x max_time]
        context_size = self.rnn_size*2
        att_prob = attention_self(bi_rnn_out, self.in_len, context_size, context_size)
        sentence = tf.reduce_sum(bi_rnn_out * tf.expand_dims(att_prob, -1), axis=1)  # [batch x context_size]

        fc1 = tf.layers.dense(sentence, self.rnn_size, activation=tf.nn.relu)
